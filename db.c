	...
	for (;;)
	{
	   ...
	   */
		...
		if(alreadyExists)
		{
			...
			snprintf(buf, MAX_STRING_LENGTH, "NewMob: %ld, iHash: %d, Next: %s", pMobIndex->vnum, iHash, pMobIndex->next->player_name);
			log_string(buf);

			snprintf(buf, MAX_STRING_LENGTH, "OldMob: %ld, iHash: %d, Next: %s", pMobExists->vnum, iHash, pMobExists->next->player_name);
			log_string(buf);
