    ...
    for ( pReset = pArea->reset_first; pReset != NULL; pReset = pReset->next )
    {
	   ...
	   case 'E':
		  ...
		  if ( mob->pIndexData->pShop != NULL )
		  {
			 ...
			 case ITEM_BOMB: olevel = number_range( 5, 100 ); break;
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
    ...
    if(mob->armor < -800) mob->armor = -800;
    ...
    /* Palmer added here  */
    if(pMobIndex->hitnodice <= 0 && pMobIndex->hitsizedice <= 0
	   && pMobIndex->hitplus <=0)
    {
...
void do_memory( CHAR_DATA *ch, char *argument )
{
    ...
    snprintf( buf, MAX_STRING_LENGTH, "Affects %5d\n\r", top_affect    ); send_to_char_formatted( buf, ch );
    snprintf( buf, MAX_STRING_LENGTH, "Areas   %5d\n\r", top_area      ); send_to_char_formatted( buf, ch );
    snprintf( buf, MAX_STRING_LENGTH, "RmTxt   %5d\n\r", top_rt        ); send_to_char_formatted( buf, ch );
    snprintf( buf, MAX_STRING_LENGTH, "ExDes   %5d\n\r", top_ed        ); send_to_char_formatted( buf, ch );
    snprintf( buf, MAX_STRING_LENGTH, "Exits   %5d\n\r", top_exit      ); send_to_char_formatted( buf, ch );
    snprintf( buf, MAX_STRING_LENGTH, "Helps   %5d\n\r", top_help      ); send_to_char_formatted( buf, ch );
    snprintf( buf, MAX_STRING_LENGTH, "Mobs    %5d\n\r", top_mob_index ); send_to_char_formatted( buf, ch );
    snprintf( buf, MAX_STRING_LENGTH, "Objs    %5d\n\r", top_obj_index ); send_to_char_formatted( buf, ch );
    snprintf( buf, MAX_STRING_LENGTH, "Resets  %5d\n\r", top_reset     ); send_to_char_formatted( buf, ch );
    snprintf( buf, MAX_STRING_LENGTH, "Rooms   %5d\n\r", top_room      ); send_to_char_formatted( buf, ch );
    snprintf( buf, MAX_STRING_LENGTH, "Shops   %5d\n\r", top_shop      ); send_to_char_formatted( buf, ch );
