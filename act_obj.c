...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( ch->level + number_range(1,20) < victim->level          )
	   ...
	   ||   ( !IS_NPC(victim) && IS_IMMUNE(victim,IMM_STEAL) && number_percent() > 5 )
...
int place_object( CHAR_DATA *ch, int object )
{
    ...
    for( ; ; )
    {
	   ...
	   if( pObjIndex == NULL) continue;
	   if( !IS_SET( pObjIndex->wear_flags, ITEM_TAKE)) continue;
	   if (IS_SET(obj->extra_flags, ITEM_NOQUESTCARD)) continue; 
	   if( IS_SET(obj->extra_flags, ITEM_VANISH)) continue;
	   if( !CAN_WEAR(obj, ITEM_TAKE)) continue;
	   if ( IS_SET( obj->extra_flags, ITEM_AUTOCLAIM)) continue;
