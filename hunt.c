...
void do_hunt( CHAR_DATA *ch, char *argument )
{
    ...
    */
    ...
    if (IS_NPC(victim)) snprintf( buf,  MAX_STRING_LENGTH, "%s is %s from here.",capitalize(victim->short_descr), dir_name[direction] );
    else snprintf( buf,  MAX_STRING_LENGTH, "%s is %s from here.", victim->name, dir_name[direction]); 
...
void hunt_victim( CHAR_DATA *ch )
{
    ...
    */
    for( found = 0, tmp = char_list; tmp && !found; tmp = tmp->next )
	   ...
	   {
		  if (number_bits(6) == 0) ch->hunting = NULL;
		  return;
	   }
