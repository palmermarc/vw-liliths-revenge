			...
			for (i = 0; i <= 5; i++)
			{
				...
				if (exit_ok(exitp) && (thru_doors ? GO_OK_SMARTER : GO_OK))
				{
				    ...
				    count determines total breadth and depth */
					   ...
					   hash_enter( &x_room, tmp_room, (void*)(intptr_t)hash_find(&x_room,q_head->room_nr) == -1) ? (void*)(intptr_t)(i+1) : hash_find(&x_room,q_head->room_nr));
						}
					}
					else
					{
						...
						if ((void *)(intptr_t)hash_find(&x_room, tmp_room) == -1)
						{
							if (x_room.buckets)
							{
						...
						else
						{
							...
							i = (void *)(intptr_t)hash_find(&x_room, tmp_room);
							if (x_room.buckets)
							{
								/* junk left over from a previous track */
								destroy_hash_table(&x_room, donothing);
