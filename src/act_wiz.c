...
void do_watcher(CHAR_DATA *ch, char *argument)
{
	...
	if (!IS_SET(ch->act, PLR_WATCHER) || (ch->level > 3) || (ch->level < 2) || (ch->trust > 0))
	{
		send_to_char("Huh?\n\r", ch);
		return;
	}
	...
	if (!IS_SET(ch->act, PLR_WIZINVIS))
	{
		act("$T.", ch, NULL,
			(ch->pcdata != NULL && ch->pcdata->bamfout[0] != '\0')
				? ch->pcdata->bamfout
				: "Someone tell this dolt to set their bamfout!",
			TO_ROOM);
	}
	...
	if (!IS_SET(ch->act, PLR_WIZINVIS))
	{
		act("$T.", ch, NULL,
			(ch->pcdata != NULL && ch->pcdata->bamfin[0] != '\0')
				? ch->pcdata->bamfin
				: "Someone tell this dolt to set their bamfin!",
			TO_ROOM);
	}
...
void do_trust(CHAR_DATA *ch, char *argument)
{
	...
	if (!str_cmp(arg2, "none"))
		level = 0;
	else if (!str_cmp(arg2, "justicar"))
		level = 4;
	else if (!str_cmp(arg2, "elder"))
		level = 5;
	else if (!str_cmp(arg2, "seer"))
		level = 6;
	else if (!str_cmp(arg2, "oracle"))
		level = 7;
    ...
    */
	...
	if (!str_cmp(arg2, "level") && ch->level == LEVEL_CODER)
	{
		...
		if (!str_cmp(arg3, "mortal"))
			...
			value = 3;
		else if (!str_cmp(arg3, "justicar"))
			value = 4;
		else if (!str_cmp(arg3, "elder"))
			value = 5;
		else if (!str_cmp(arg3, "seer"))
			value = 6;
		else if (!str_cmp(arg3, "oracle"))
			value = 7;
		else if (!str_cmp(arg3, "god"))
			value = 8;
	...
	if (!str_cmp(arg2, "beast"))
	{
		...
		if (IS_JUDGE(ch))
		{
			...
			else if (value != 0 && IS_SET(victim->immune, IMM_SUNLIGHT))
...
void do_qset(CHAR_DATA *ch, char *argument)
{
	...
	if (!str_cmp(arg2, "chwear"))
	{
		if (obj->chpoweron != NULL)
			strncpy(buf, obj->chpoweron, MAX_STRING_LENGTH);
		if (!str_cmp(arg3, "clear"))
		{
	...
	else if (!str_cmp(arg2, "chrem"))
	{
		if (obj->chpoweroff != NULL)
			strncpy(buf, obj->chpoweroff, MAX_STRING_LENGTH);
		if (!str_cmp(arg3, "clear"))
		{
	...
	else if (!str_cmp(arg2, "chuse"))
	{
		if (obj->chpoweruse != NULL)
			strncpy(buf, obj->chpoweruse, MAX_STRING_LENGTH);
		if (!str_cmp(arg3, "clear"))
		{
	...
	else if (!str_cmp(arg2, "victwear"))
	{
		if (obj->victpoweron != NULL)
			strncpy(buf, obj->victpoweron, MAX_STRING_LENGTH);
		if (!str_cmp(arg3, "clear"))
		{
	...
	else if (!str_cmp(arg2, "victrem"))
	{
		if (obj->victpoweroff != NULL)
			strncpy(buf, obj->victpoweroff, MAX_STRING_LENGTH);
		if (!str_cmp(arg3, "clear"))
		{
	...
	else if (!str_cmp(arg2, "victuse"))
	{
		if (obj->victpoweruse != NULL)
			strncpy(buf, obj->victpoweruse, MAX_STRING_LENGTH);
		if (!str_cmp(arg3, "clear"))
		{
...
void oset_affect(CHAR_DATA *ch, OBJ_DATA *obj, int value, int affect, bool is_quest)
{
	...
	if (!IS_JOKER(ch) && (obj->questmaker == NULL || str_cmp(ch->name, obj->questmaker)) && !is_quest)
	{
...
void do_claim(CHAR_DATA *ch, char *argument)
{
	...
	if (ch->exp < 5000)
	{
		send_to_char("It costs 5000 exp to claim ownership of an item.\n\r", ch);
		return;
	}
